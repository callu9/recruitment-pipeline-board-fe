...
import { APPLICANT_OWNERS, APPLICANT_ROLES, type Applicant, type ApplicantEvaluation, type ApplicantOwner, type ApplicantRole, type ApplicantStage, type EvaluationType, type Position, type SubmitApplicantFeedbackRequest } from './features/recruitment-board/model/applicant.types'
import { getAllowedNextStages, getCurrentStageEvaluation, getForwardActionLabel, getLocalDateString, isTerminalStage, STAGES } from './features/recruitment-board/model/stages'
...
function stageLabelForEvaluation(type: EvaluationType) { return { SCREEN: '서류검토', INTERVIEW: '면접', FINAL: '처우협의' }[type] }
...
function FeedbackForm({ evaluation, isPending, error, onSubmit }: {
  ...
  return <form className={styles.feedbackForm} aria-label={`${stageLabelForEvaluation(evaluation.type)} 피드백`} aria-busy={isPending} onSubmit={(event) => {
    event.preventDefault()
    onSubmit({ reviewer, score: Number(score), comment })
  }}>
    <label>평가자<select value={reviewer} disabled={isPending} onChange={(event) => setReviewer(event.target.value as ApplicantOwner)}>{APPLICANT_OWNERS.map((owner) => <option key={owner}>{owner}</option>)}</select></label>
    <label>점수<input type="number" min="0" max="100" required value={score} disabled={isPending} onChange={(event) => setScore(event.target.value)} /></label>
    <label>코멘트<textarea required value={comment} disabled={isPending} onChange={(event) => setComment(event.target.value)} /></label>
    {error && <p role="alert">{error}</p>}
    <button type="submit" className={styles.primaryButton} disabled={isPending}>{isPending ? '저장 중' : '피드백 저장'}</button>
  </form>
}

function ApplicantDetail({ applicant, positions, onClose, onMove, onReject, onCorrection, onSubmitFeedback, isPending, feedbackPending, feedbackError }: { applicant: Applicant; positions: Position[]; onClose: () => void; onMove: (stage: ApplicantStage, trigger?: HTMLButtonElement) => void; onReject: (trigger?: HTMLButtonElement) => void; onCorrection: (trigger?: HTMLButtonElement) => void; onSubmitFeedback: (evaluationId: string, values: SubmitApplicantFeedbackRequest) => void; isPending: boolean; feedbackPending: boolean; feedbackError?: string }) {
  const dialogRef = useRef<HTMLDialogElement>(null)
  ...
  const currentEvaluation = getCurrentStageEvaluation(applicant)
  ...
  return <dialog ref={dialogRef} className={styles.detailPanel} role="dialog" aria-modal="true" aria-labelledby={titleId} onCancel={(event) => { event.preventDefault(); close() }} onKeyDown={(event) => { if (event.key === 'Escape') { event.preventDefault(); close() } }}>
    ...
    <section className={styles.detailSection} aria-labelledby={`${titleId}-evaluation`}><h3 id={`${titleId}-evaluation`}>평가</h3><div className={styles.evaluationList}>{(applicant.evaluations ?? []).map((evaluation) => <div className={styles.evaluation} key={evaluation.id}>
      ...
      {evaluation.status === 'SUBMITTED' && evaluation.comment
        ...
        : evaluation.id === currentEvaluation?.id
          ? <FeedbackForm evaluation={evaluation} isPending={feedbackPending} error={feedbackError} onSubmit={(values) => onSubmitFeedback(evaluation.id, values)} />
          : null}
    </div>)}</div></section>
...
function App() {
  ...
  const { submitFeedback, pendingIds: feedbackPendingIds } = useSubmitApplicantFeedback({ onError: ({ applicantId, evaluationId }) => setFeedbackSaveError({ applicantId, evaluationId, message: '피드백을 저장하지 못했습니다. 다시 시도해 주세요.' }), onSuccess: (applicant, { evaluationId }) => { setFeedbackSaveError((current) => current?.applicantId === applicant.id && current.evaluationId === evaluationId ? null : current); setFeedbackSuccess(`${applicant.name}님의 피드백을 저장했습니다.`) } })
  ...
  const busyApplicantIds = new Set([...pendingIds, ...feedbackPendingIds])
