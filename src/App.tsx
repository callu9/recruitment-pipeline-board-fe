...
function StageInlineControl({ applicant, isPending, onMove }: { applicant: Applicant; isPending: boolean; onMove: (stage: ApplicantStage) => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (targetStage) onMove(targetStage)
  }
  ...
  return (
    <dialog ref={dialogRef} className={styles.confirmationDialog} aria-labelledby={titleId} onCancel={(event) => { event.preventDefault(); dialogRef.current?.close() }}>
      ...
      <div className={styles.dialogActions}>
        ...
        <button type="button" className={styles.primaryButton} onClick={onConfirm}>확인</button>
      </div>
    </dialog>
  )
}
...
function ApplicantDetail({ applicant, onClose, onMove, isPending }: { applicant: Applicant; onClose: () => void; onMove: (stage: ApplicantStage) => void; isPending: boolean }) {
  useEffect(() => {
  ...
  return (
    <aside className={styles.detailPanel} aria-label={`${applicant.name} 상세 정보`} tabIndex={-1}>
      ...
      <section className={styles.detailSection} aria-labelledby="detail-summary"><h3 id="detail-summary">기본 정보</h3><dl className={styles.detailList}>
        <div><dt>담당자</dt><dd>{applicant.owner ?? '미지정'}</dd></div><div><dt>포지션</dt><dd>{applicant.positionId ?? '미지정'}</dd></div><div><dt>지원일</dt><dd>{fullDateLabel(applicant.appliedAt.slice(0, 10))}</dd></div><div><dt>다음 액션</dt><dd>{applicant.nextAction ?? '확인 필요'}</dd></div><div><dt>마감일</dt><dd>{fullDateLabel(applicant.dueDate)}</dd></div>
      </dl></section>
...
function ApplicantsView({ applicants, filters, setFilters, onSelect, selectedId, pendingIds, onMove, positions }: { applicants: Applicant[]; filters: WorkspaceFilters; setFilters: (next: WorkspaceFilters) => void; onSelect: (id: string, trigger?: HTMLButtonElement) => void; selectedId: string | null; pendingIds: ReadonlySet<string>; onMove: (applicant: Applicant, stage: ApplicantStage) => void; positions: Position[] }) {
  ...
  return <section className={styles.view} aria-labelledby="applicants-heading">
    ...
    {filtered.length === 0 ? <div className={styles.emptyState}><h3>조건에 맞는 지원자가 없습니다.</h3><p>검색어나 필터를 초기화해 다시 확인하세요.</p><button type="button" onClick={() => setFilters(EMPTY_FILTERS)}>필터 초기화</button></div> : <div className={styles.tableWrap}><table className={styles.applicantTable}><caption className={styles.srOnly}>지원자 목록</caption><thead><tr><th scope="col"><span className={styles.srOnly}>선택</span>□</th><th scope="col">지원자</th><th scope="col">단계</th><th scope="col">담당자</th><th scope="col">다음 액션</th><th scope="col">일정</th><th scope="col"><span className={styles.srOnly}>액션</span></th></tr></thead><tbody>{filtered.map((applicant) => <tr key={applicant.id} className={selectedId === applicant.id ? styles.selectedRow : ''}>
      ...
      <td><StageInlineControl applicant={applicant} isPending={pendingIds.has(applicant.id)} onMove={(stage) => onMove(applicant, stage)} /></td>
    </tr>)}</tbody></table></div>}
  </section>
}
...
function CalendarView({ applicants, onSelect }: { applicants: Applicant[]; onSelect: (id: string, trigger?: HTMLButtonElement) => void }) {
  ...
  const weekDays = Array.from({ length: 7 }, (_, index) => { const day = new Date(`${WORKSPACE_TODAY}T00:00:00`); day.setDate(day.getDate() + index); return day.toISOString().slice(0, 10) })
  const unscheduled = getUnscheduledApplicants(applicants)
...
function App() {
  ...
  const [confirmation, setConfirmation] = useState<{ applicant: Applicant; targetStage: ApplicantStage } | null>(null)
  ...
  function requestMove(applicant: Applicant, targetStage: ApplicantStage) {
    if (targetStage === 'HIRED' || targetStage === 'REJECTED') setConfirmation({ applicant, targetStage })
  ...
  return <main className={styles.shell}>
    ...
    {selectedApplicant && <ApplicantDetail applicant={selectedApplicant} onClose={() => { setSelectedId(null); detailTrigger.current?.focus() }} onMove={(stage) => requestMove(selectedApplicant, stage)} isPending={pendingIds.has(selectedApplicant.id)} />}
    {confirmation && <StageConfirmationDialog applicant={confirmation.applicant} targetStage={confirmation.targetStage} onCancel={() => setConfirmation(null)} onConfirm={confirmMove} />}
  </main>
}
