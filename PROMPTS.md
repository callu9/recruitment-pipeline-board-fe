...
- `DECISIONS.md`의 의사결정을 프로젝트 과제 기준인 3~5개로 정리한다.
- 핵심 사용자 흐름·데이터 안전성·접근성에 직접 영향을 주는 5개 판단만 남긴다.
- 구현 세부 판단은 `TECH_SPEC.md`의 해당 절로 옮긴다.
- 구현 계획의 결정 참조가 삭제된 번호를 가리키지 않게 한다.

### 프롬프트 1 — 결정 내용 우선순위 검토

```text
DECISIONS.md 내용이 5개 이하로 조정 필요한 것으로 이해함. 내용 확인 후 비개발자 및 사용자가 이해하기 쉽도록 풀어서 설명하여 내용 목록화한 후, 우선순위 높은 5개 꼽아줘
```

### 프롬프트 2 — 문서 정리와 커밋 요청

```text
위 5개로 남기고, 나머지 D-001, D-006, D-008, D-009, D-013은 `TECH_SPEC.md` 옮기도록 함.
파일 수정 후 커밋,푸시까지 진행
```

### AI 출력 요지

- 단계 이동 정책, Query cache 단일 소유, MSW·localStorage mock API, 엔티티 단위 롤백·중복 요청 차단, 카드별 live region을 핵심 5개로 제안했다.
- D-001, D-006, D-008, D-009, D-013의 구현 세부를 TECH_SPEC의 기술 기준·API 계약·검색/확장 범위·저장소 초기화 절로 옮겼다.
- 구현 계획의 삭제된 결정 번호 참조를 남은 결정 또는 TECH_SPEC 절 참조로 갱신했다.

### 리뷰 / 검증

- 사용자 확인: unstaged diff와 검증 결과를 확인한 뒤 커밋·푸시를 요청했다.
- 확인: `DECISIONS.md`에 D-002, D-003, D-004, D-005, D-010의 다섯 항목만 남았고, 구현 계획에는 삭제된 결정 번호 참조가 남지 않았다.
- 자동 검증: `npm run lint`, `npm run test`(8개 파일·76개 테스트), `npm run build`, `git diff --check`가 모두 통과했다.
- 알려진 제한: build는 기존 500kB 초과 chunk 경고를 계속 출력한다. 이번 문서 정리 범위에서는 번들 구조를 변경하지 않았다.
...
- 예정 메시지:
  ...
  ```text
  docs(decision-record-consolidation): 핵심 결정 5개로 정리

  - 구현 세부 판단을 TECH_SPEC에 이전
  - 구현 계획의 삭제된 결정 참조를 갱신
