...
2. Before drafting, inspect the immediately previous feature section. If its hash is missing, replace its uncommitted status with `- 해시: <short-hash>` only when exactly one commit reachable from `HEAD` has that scope; otherwise report the ambiguity without guessing. Leave the current scope uncommitted until the next feature records it.
...
8. Preview the complete current section and any previous-section hash backfill without editing files.
