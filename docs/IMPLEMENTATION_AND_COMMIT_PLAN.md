...
5. 구현 후 diff를 정독하고 요구 누락, 잘못된 가정, 불필요한 의존성, 이해하지 못한 코드를 확인한다.
6. 관련 테스트와 `npm run lint`, `npm run test`, `npm run build`를 실행한다.
7. 자동 검증 결과와 unstaged diff, 사용자 수동 검증 시나리오를 보고하고 기다린다.
8. 사용자 검증 결과를 받거나 미검증 범위의 명시적 수용을 확인한다.
9. 수정이 필요하면 반영한 뒤 5단계부터 반복한다.
10. 안정된 diff에서 별도 요청 없이 `prompt-record`로 현재 scope의 실제 검증 결과를 작성한다. 이전 scope와 해시는 수정하지 않는다.
11. 기록만 변경됐다면 prompt 계약 테스트와 `git diff --check`만 실행한다.
12. 관련 파일만 stage한다.
13. staged diff를 다시 확인하고 사용자에게 보고한다.
14. 명시적 커밋 요청을 받은 뒤 기능 코드·관련 테스트·기록을 한 커밋으로 남긴다.
