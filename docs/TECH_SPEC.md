...
- Vite 기반 SPA를 사용한다. 이번 제품에는 서버 렌더링·인증·복잡한 라우팅이 없으므로, Next.js의 서버 기능보다 낙관적 업데이트와 실패 복구 검증에 시간을 쓴다.
- 기능별 스타일은 CSS Modules로 두고 전역 CSS는 reset과 공통 토큰에만 사용한다. 별도 CSS-in-JS 의존성이나 런타임 설정은 추가하지 않는다.
...
- PATCH body는 stage 문자열 하나만 받으므로 입력 형태와 stage 값은 검증하되, 별도 요청 크기 제한은 두지 않는다. 실제 서버나 큰 payload가 추가되면 인프라 수준 제한과 함께 다시 검토한다.
...
- 시드 데이터는 같은 크기에서 같은 ID·이름·단계가 나오도록 index 기반으로 결정적으로 생성한다. 작은 고정 한글·영문 이름 목록을 순환해 기본 seed에서 영문 대소문자 무시 검색을 수동 확인할 수 있게 한다. 실패 시뮬레이션의 난수와 시드 데이터 생성을 분리한다.
- 유효한 기존 저장 데이터는 그대로 반환한다. 혼합 seed는 저장 키가 없거나 저장 데이터가 손상된 경우에만 생성·저장하며, 저장 키 변경·migration·자동 reset은 추가하지 않는다.
...
- GET 응답은 별도 `total` 없이 `Applicant[]`를 반환한다.
- 실제 다중 사용자 충돌 검출에 쓰이지 않는 `version`과 별도 정렬용 `stageUpdatedAt`은 두지 않는다.
- Undo는 단계 변경에 연결될 수 있는 외부 알림 상태와 불일치할 수 있어 구현하지 않는다.
- 서버 페이지네이션, 실제 다중 사용자 충돌 처리, 1,000건 렌더링 병목이 확인되면 그때 필요한 계약과 최적화를 추가한다.
